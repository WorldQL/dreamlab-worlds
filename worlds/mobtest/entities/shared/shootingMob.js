...
import { createProjectile } from "./dist/entities/client/projectile.js";
...
export const createHittableMob = createSpawnableEntity(
  ({ uid, tags, transform, zIndex }) => {
    ...
    let direction = 1;
    const shootCooldown = 3; // 3 seconds
    let shootTimer = 0;
    ...
    return {
      ...
      onPhysicsStep({ delta }, { game, netClient }) {
        ...
        shootTimer = Math.max(shootTimer - delta, 0);
        if (shootTimer === 0) {
      ...
      shoot(game) {
        const direction2 = 1; // Assume 1 for right, -1 for left
        game.spawn({
          entity: createProjectile,
          args: [this.transform, 10, direction2], // Assume a radius of 10 for the projectile
