  ...
  return {
    ...
    initRenderContext: async (_, { camera, stage }) => {
      ...
      return {
        ...
        gfx: gfxBounds,
        gfxHealthBorder,
        gfxHitBox: gfxHittest,
        healthContainer: ctrHealth,
        gfxHealthAmount,
        sprite,
      }
    },
    ...
    teardownRenderContext({ container, healthContainer: ctrHealth, sprite }) {
    ...
    onArgsUpdate(path, previous, data, render) {
      if (path === 'width' || path === 'height') {
        ...
        if (render) {
          ...
          render.healthContainer.position.y = -args.height / 2 - 30
